...
export const addTemplatePersonalized = async (templateState: TemplateState) => {
  ...
  const loveStoryData = loveStoryTemp.map(({ storyFile, ...rest }: any) => rest);
  ...
  storyFile.map((story, index) => {
    if (templateState.loveStorySaved && loveStoryData) loveStoryData[index].storyPhoto = story;
    else loveStoryData[index].storyPhoto = "";
  });

  // Sequential Upload for single images
  if (templateState.coverFile) coverFile = await handleUploudImage(templateState.coverFile);
  if (templateState.groom.file) groomFile = await handleUploudImage(templateState.groom.file);
  if (templateState.bride.file) brideFile = await handleUploudImage(templateState.bride.file);
  if (templateState.qrisFile) qrisFile = await handleUploudImage(templateState.qrisFile);
